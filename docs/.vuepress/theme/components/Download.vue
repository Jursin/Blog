...
type DownloadSourceId = 'github.com' | 'wget.la'
...
const selectedDownloadSource = ref<DownloadSourceId>('wget.la')
...
async function fetchLatestRelease() {
    ...
    try {
        ...
        const responses = await Promise.all(repos.map(r => fetch(`https://api.github.com/repos/Jursin/${r}/releases?per_page=1`, {
            headers: {
                'Accept': 'application/vnd.github.v3+json',
                'User-Agent': 'plume-template/1.0'
            },
        ...
        releases.value = valid.map((rel: any, idx: number) => ({
            ...rel,
            _repo: repos[idx]
        }))
    } catch (error) {
...
const currentRelease = computed(() => {
    ...
    return releases.value.slice().sort((a: any, b: any) => new Date(b.published_at).getTime() - new Date(a.published_at).getTime())[0]
})
...
const filteredAssets = computed(() => {
    ...
    for (const rel of releases.value) {
        ...
        for (const asset of rel.assets) allAssets.push({ ...asset, _repo: rel._repo })
    }
    ...
    <div class="download-container">
        ...
        <div class="release-info">
            ...
            <div class="release-header">
                <img src="/icon/Schedule-Vela.png" alt="logo" class="logo">
                <div class="release-title-row">
                    ...
                    <template v-else>
                        ...
                        <span class="release-date">{{ currentRelease ? new
                            Date(currentRelease.published_at).toLocaleDateString(dateLocale) : '' }}</span>
                    </template>
                </div>
            </div>
                            ...
                            <button class="dropdown-trigger" @click="isSourceDropdownOpen = !isSourceDropdownOpen"
                                ...
                                <span class="dropdown-content">
                                    <template v-if="currentDownloadSource.icon">
                                        <img v-if="isGithubSource(currentDownloadSource.id)" src="/icon/github-dark.png"
                                            class="source-icon github-light" />
                                        <img v-if="isGithubSource(currentDownloadSource.id)"
                                            src="/icon/github-light.png" class="source-icon github-dark" />
                                        <img v-else :src="currentDownloadSource.icon" class="source-icon" />
                                    </template>
                            ...
                            <div class="dropdown-menu">
                                <button v-for="source in DownloadSources" :key="source.id" class="dropdown-item" :class="{
                                    ...
                                    <template v-if="source.icon">
                                        <img v-if="isGithubSource(source.id)" src="/icon/github-dark.png"
                                            class="source-icon github-light" />
                                        <img v-if="isGithubSource(source.id)" src="/icon/github-light.png"
                                            class="source-icon github-dark" />
                                        <img v-else :src="source.icon" class="source-icon" />
                                    </template>
                                    <span class="source-info">
            ...
            <div class="download-section">
                <h2 style="font-weight: bold;">文件列表</h2>
                        ...
                        <div class="asset-info">
                            ...
                            <div class="asset-meta">
                                ...
                                <span class="asset-sha-wrapper">
                                    <span class="asset-sha" :title="`${getAssetSha256(asset)}`">sha256:{{
                                        getAssetSha256(asset)
                                        }}</span>
                                    <button type="button" class="sha-copy-button" @click="copyAssetSha256(asset)">
                                        <Icon v-if="copiedShaAssetId === asset.id" name="octicon:check-16"
                                            color="#1a7f37" />
